...
| 2.9h | Mapa de decisión (reemplaza la matriz de burbujas): una fila por canal, franjas cae/estable/crece y línea de peso medio | `ui/comercial._matriz_figura` | `UMBRAL_MOVIMIENTO` | 🔴 — cuando los canales se mueven parecido, la matriz apilaba todas las burbujas en una columna y se tapaban. Con una fila por canal no pueden montarse. El hover y la línea de cero se fijan DESPUÉS de `_base`, que impone hover unificado y quita la línea de cero. |
| 2.8d | Planes de mejora: hoja de ruta en tabla, filtro por urgencia, planes plegables y pasos como casillas con avance | `ui/planes.py` (`_hoja_de_ruta`, `_plan`) | `core/planes.generar` | 🟡 — no se quitó información: situación, evidencia, por qué, pasos y medición siguen en cada plan. La clave de cada casilla sale de un hash del título y la situación, no de la posición, para que el avance no se pierda al cambiar el filtro. |
| 2.9c | Jugadas ordenadas por impacto en la moneda del archivo | `core/comercial.oportunidades` | — | 🟡 — una jugada por canal: dos líneas del mismo canal compiten entre ellas y desplazan a uno que no aparece nunca. |
