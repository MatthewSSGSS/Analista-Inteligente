...
| 0.7 | Informes ejecutivos con más formas: meses combinados sobre Altas/Meta/Cum, meses intercalados con «% Participación», listados normales dentro del informe (zonificación), títulos de una palabra («FWA») y agentes sin encabezado («BMS», «RYL») que NO son títulos; una sección por (título, qué agrupan las filas) y nombre de medida desde el título («Ventas @», «Altas @», «Cumplimiento») | `core/informe._tablas_meses_agrupados`, `_tablas_planas`, `_titulo_corto`, `_es_etiqueta`, `_nombrar_medida`, `_nombrar_medidas_sin_nombre`, `_dimension_por_titulo` · `leer_informe` | `core/loader.load_workbook` | 🔴 — con el informe real «DASH BOARD NUEVO EJECUTIVO DE CALLE»: la tabla de OTTS por región salía solo con las altas de abril, el listado de zonificación desaparecía y FWA salía como «RANKING SUPERVISORES» con solo enero. `_tablas_meses_agrupados` va ANTES que el detector horizontal: si no, este toma la fila de meses y pierde las medidas. `_es_etiqueta` existe porque `_numero("R1")` da 1 y la columna R1…R5 no parecía de etiquetas. Un título corto sobre «Mes | ene | feb» nombra a una entidad, no titula. `tests/informe_ejecutivo_test.py`. |
| 0.8 | En tablas de informe, una celda vacía es «sin reportar», no cero; la variación compara el último mes CON dato | `core/cleaner.clean(faltantes_son_cero=)` · `profile_sheet(context['faltantes_son_cero'])` · marca `faltantes_son_cero: False` en `leer_informe` · `core/dashboard_engine` (numeric_valid + `sum(min_count=1)`) · `core/executive._monthly` | — | 🔴 — el panel decía «Ingresos Comisión retrocedió 100%» porque agosto no estaba reportado. Las hojas normales siguen convirtiendo faltantes a 0 como siempre. |
| 0.9 | Tablas idénticas copiadas en otra hoja se muestran una vez (con aviso); imágenes avisadas por el nombre de su sección; Inicio lista «Qué se leyó del archivo» tabla por tabla | `core/loader._misma_tabla` · `core/informe._filas_de_imagenes` · `ui/home.filas_contenido/_contenido_del_archivo` | — | 🟡 — «tiene 2 imágenes» no decía qué se perdía; «RANKING DE LOS JEFES es una imagen» sí. |
| 0.10 | NIT, cédula, NUIP, RUT/RUC siempre son identificadores, aunque se repitan | `core/schema.DOC_ID_RE` | — | 🟡 — `ID_RE` exige que la mayoría de valores sean distintos; en un listado de zonas el NIT de un agente se repite y se sumaba como métrica. |
| 0.11 | Los gráficos rotulan con el nombre de la columna, no con el concepto («Altas», no «Cantidad») | `visualization/charts._label` | — | 🟡 — igual que las otras copias de `_label`. Con varias columnas de altas, todas decían «Cantidad». |
| 0.12 | Imágenes pegadas en el Excel (capturas de tablas y de gráficos de líneas con sus valores escritos) leídas con OCR LOCAL, revisadas y sumadas al panel como una tabla más | `core/imagen_ocr.py` (`imagenes_del_libro`, `leer_textos`, `interpretar_tabla`, `interpretar_grafico`, `restaurar_nombre`, `a_tabla_larga`) · `core/loader` (`wb['imagenes']` con su sección) · `ui/imagenes.render_imagenes` en 🏠 Inicio | `rapidocr_onnxruntime` (modelos incluidos en el paquete; no envía ni descarga nada al usarse) · `packages.txt` (libgl1 para OpenCV en Linux) | 🔴 — NADA sale a servicios externos: fue una condición explícita. Nunca se analiza a ciegas: leer (botón) → revisar/editar junto a la imagen → «Usar». El clasificador de orientación del OCR se APAGA (`use_angle_cls=False`): volteaba «90 %» a «% 06». Se lee a dos escalas y se suman los textos (cada pasada sola perdía etiquetas distintas). Cada número va a su columna/mes por POSICIÓN, no por orden de lectura: así las celdas vacías no corren los valores. El modelo pierde espacios y Ñ en mayúsculas: los nombres se restauran contra los textos del libro. En gráficos, la serie se decide por el color de la leyenda (muestreo corto y prefiriendo el píxel saturado: con 30 px se tomaba el negro del texto vecino) y la línea se busca primero en el centro de la etiqueta y solo después en todo su ancho. Verificado contra el informe real: 48/48 valores del gráfico, 8×12 jefes y 26×12 supervisores con sus 12 celdas vacías. `tests/imagen_ocr_test.py` (se salta si RapidOCR no está). **Incidente corregido:** con RapidOCR 1.4.4 la opción del giro se llama `use_cls` (no `use_angle_cls`), quedó encendida sin error y JIMENEZ MESTRA salió con 6 y 66 donde la imagen decía 90 % y 99 %. Ahora hay tres defensas independientes de la versión: apagar ambas opciones y pasar `use_cls=False` en la llamada (`_motor`, `_ejecutar`); releer toda cifra con el % delante y, si no queda clara, dejar la celda VACÍA y marcada, nunca adivinada (`_corregir_giradas`); y revisar coherencia fila contra sus meses y contra su Total (`_revisar_coherencia`). Con dudas, «Usar esta tabla» queda bloqueado hasta confirmar la revisión. La 1.4.4 además pega el círculo de la leyenda al nombre («OAsesores»): el color se toma de la mancha uniforme más grande (`_marcador_leyenda`). Verificado celda por celda en 1.2.3 y 1.4.4: jefes 96/96, supervisores 312/312, gráfico 48/48. `requirements.txt` limitado a <1.5 hasta repetir esa verificación. |
...
| 1.4c | "📊 Métrica a analizar": todo el panel (veredicto, tarjetas, hallazgos, comparaciones, estrategia, planes, gráficos, predicciones) se calcula sobre la métrica elegida; agregar una columna numérica en "🧰 Más columnas" la elige sola, y la franja superior dice "📊 Analizando …" | `app.py` (selector + `schema["metrica_preferida"]`) · `ui/filtros.metrica_recien_elegida` · respetada en `visualization/charts.metric_candidates`, `core/executive.primary_metric`, `core/dashboard_engine`, `core/insights`, `core/performance.analyze`, `core/diagnostics.diagnosticar`, `core/comercial.matriz_comercial`, `core/universal_analysis.choose_metric` | — | 🔴 — antes filtrar por "Altas Eje" dejaba todo el análisis en "ALTAS". |
| 1.4d | La meta se empareja con SU métrica: "Meta Altas Eje" no se usa para "ALTAS", y una meta de escala imposible (112.4K contra 1.3B) se descarta | `core/performance.columna_meta` | nombre de la meta y proporción resultado/meta entre 2% y 5000% | 🔴 — el panel llegó a decir "Mejor en cumplimiento: 0% · 112.4K de una meta de 1.3B". |
| 1.4e | Controles del menú lateral: sin la cajita blanca junto a cada etiqueta elegida, etiquetas en píldora, íconos compactos; la tarjeta del mejor muestra nombres largos en tamaño de texto | `ui/styles/theme.py` (bloque sidebar `[data-baseweb="select"]`) · `ui/executive._render_kpi` | — | 🟢 |
...
| 2.7z | Descripción (modo Analista) presentada de nuevo: UNA sola lista de hallazgos («Qué revisar primero» + «Contexto, sin acción inmediata»), el botón de cada hallazgo pegado a su tarjeta y con nombre («Abrir el análisis de R4»), cinco cifras en una fila pareja y el líder («Mejor en cumplimiento de meta · R1 · 109%») en su propia franja a todo el ancho | `ui/dashboard._hallazgos_panel` (reemplaza a `_alerts_panel` + `_insights_panel`) · `ui/dashboard._universal_kpi_grid` | `core/executive.build_alerts` | 🔴 — la pestaña mostraba las MISMAS cuatro tarjetas dos veces («Alertas inteligentes» y «Hallazgos y líneas de acción»): no eran dos análisis, `build_alerts` construye las alertas A PARTIR de los hallazgos. El panel ya no repite su propio título (antes el encabezado decía «Alertas inteligentes» y la tarjeta de dentro también). Se quitaron del encabezado la línea técnica («25 registros y 5 columnas…», ahora al inicio de «Contexto y acción») y la franja «Lectura rápida», que decía lo mismo que el veredicto justo debajo. La tarjeta del líder quedaba CORTADA contra el borde por la regla global de `.kpi-card` (altura igual en toda la fila); fuera de la cuadrícula ya no compite con cifras de una línea. El atajo a «Análisis de seguimiento» pasó debajo del veredicto: era lo primero que se leía. |
...
| 2.9i | Pestaña "📊 Cuadro comparativo" (vistas Ejecutivo y Analista): elegir por qué columna comparar (vendedores, asesores, regiones…), la métrica y hasta 12 elementos; tarjetas de primero/último/distancia, ranking con la meta del 100% o el promedio como referencia, evolución mes a mes, resultado vs. meta, cuadro con cumplimiento, participación, variación y estado en palabras, lectura con nombres y descarga CSV | `core/cuadro_comparativo.py` (`opciones_de_comparacion`, `cuadro_comparativo`, `lectura`) · render `ui/cuadro_comparativo.render_cuadro_comparativo` · pestaña en `app.py` | `core/performance.columna_meta`, `core/diagnostics` (`PEOR_SI_SUBE`, `_periodo_parcial`, `METRICA_CONTEO`) | 🟡 — con meta se ordena por CUMPLIMIENTO, no por volumen: el que más vende puede ir cuarto si su meta era la más alta. Promedio, participación y posición son ENTRE LOS ELEGIDOS, no contra el archivo. A diferencia de `dimensiones_candidatas`, las opciones NO exigen que cada valor se repita: la tabla de una fila por vendedor es el caso más común. Sin meta y con carteras de tamaño muy distinto (×2) se avisa que el total mide también volumen, sin cambiar el orden. La selección solo se limpia cuando un filtro saca a alguien; si el usuario deja uno solo, se respeta y se le pide otro. `tests/cuadro_comparativo_test.py`. |
| 2.9j | "Base de la comparación" en el Cuadro comparativo: seis respuestas en palabras (a quiénes y por qué esos, qué se mide y si se suma o se promedia, contra qué —meta o promedio con su cifra—, periodo con fechas y columna, cómo se lee el color con sus umbrales, registros usados y filtros activos); además cada tarjeta, barra, subtítulo y columna del cuadro dice contra qué se mide su cifra | `core/cuadro_comparativo.base_de_comparacion` · render `ui/cuadro_comparativo._base_de_comparacion`, `_kpis`, `figura_ranking`, ayudas de columna en `render_cuadro_comparativo` | `core/filter_engine.describir_regla` | 🟡 — cada frase sale del MISMO cálculo del cuadro (umbrales `UMBRAL_PROMEDIO`, `CERCA_DE_META`, `meta_col`, `desde/hasta`), nunca de un texto fijo: si se cambia un umbral, el texto cambia solo. El rótulo de la línea de referencia va por encima del gráfico (dentro quedaba montado sobre la primera barra). La selección sugerida se pasa como `seleccion=None` para que la base diga «los 8 con mejor…» y no «elegidos por ti». Las tarjetas van en `st.container(key="cuadro_kpis")` para dejarlas crecer: la regla global de `.kpi-card` (altura 100%) cortaba la cifra bajo nombres de dos líneas. |
| 2.9k | La vara de comparación es el GRUPO COMPLETO, no los elegidos: en el Cuadro comparativo el promedio, la participación («del total de los 30»), la posición («5.º de 30») y el cumplimiento de referencia («el grupo completo va al 93%») se calculan con todos los valores de la columna (con los filtros del menú lateral); la evolución trae una línea discontinua «Promedio de los N». Sin tope de elegidos, con «Seleccionar los N» y «Solo los 8 primeros». En Analítica › Evolución, la misma línea de referencia y el hallazgo «X de los que estás viendo están sobre el promedio de los N» | `core/cuadro_comparativo.cuadro_comparativo` (`total_grupo`, `posicion_general`, `cumplimiento_grupo`, `promedio_mensual`) · `ui/cuadro_comparativo` · `core/explorador.evolucion` (`promedio_grupo`) · `ui/explorer._lente_evolucion` | — | 🔴 — pedido explícito: comparar a 3 de 30 personas contra el promedio de esas 3 no dice si van bien o mal frente al equipo (con 3 buenos elegidos, uno salía «por debajo del promedio»). Los elegidos SOLO deciden qué se muestra. Regla para cualquier vista nueva con selección. El promedio mensual se toma entre quienes tuvieron registros ese mes, para que un agente que aún no existía no baje la referencia. `tests/cuadro_comparativo_test.py` (`test_la_vara_es_el_grupo_completo`). |
...
| 2.9c | Jugadas ordenadas por impacto, con cada cifra pintada por lo que significa: rojo va mal, verde va bien, negro solo informa | `core/comercial.oportunidades` (`tono`, `partes`, `palanca_tono`) · render `ui/comercial._jugadas` | — | 🟡 — una jugada por canal: dos líneas del mismo canal compiten entre ellas y desplazan a uno que no aparece nunca. La frase se guarda por partes con su tono para que la interfaz no reinterprete el texto; `texto` sigue siendo la frase entera. Los espacios entre partes se reponen en `ui/comercial._frase_con_color` y van FUERA de la negrita: `clean_display_text` recorta los extremos de cada trozo y la frase salía pegada («Recuperar lo queR4perdió», «R3va en90%de su meta»). Cubierto por `tests/comercial_test.py`. "Sostener" es tono informativo: cuidar lo que pesa no es una alarma. |
...
| 0.4 | Hojas tipo informe (varias tablas por hoja, meses en columnas, bloques por grupo lado a lado, años combinados sobre meses, tablas resumen, filas/columnas de Total) convertidas en tablas ordenadas, una por sección con título | `core/informe.leer_informe` · integrado en `core/loader.load_workbook` | se activa por la FORMA de la hoja (fila de meses, bloques que repiten encabezado o varias tablas); una hoja normal sigue por `pivot_flatten` | 🔴 — antes la hoja salía como una sola tabla rota ("2026-01-01 00:00:00", "Columna 13", filas "R2 · Ppto"). Las secciones con título (SIN/CON PYME) quedan separadas para no sumar dos veces lo mismo. |
| 0.5 | Títulos conservados: cada tabla guarda el título bajo el que estaba ("RANKING DE LOS JEFES") y se muestra en la franja superior y en Inicio | `profile["titulo"]` · `core/informe.titulos/titulo_principal` · `app.py` (franja) · `ui/home.render_home` | — | 🟡 |
| 0.6 | Gráficos nativos de Excel leídos como datos ("Gráfico · PRODUCTIVIDAD MES A MES"); imágenes pegadas avisadas en Inicio | `core/informe.graficos_e_imagenes/leer_grafico` | copia de datos del gráfico (numCache/strCache) o, si falta, el rango de la fórmula | 🟡 — una imagen no trae números: se avisa en vez de ignorarla en silencio. Solo .xlsx/.xlsm. |
| 2.5d | Porcentajes y promedios se promedian por mes, no se suman, en veredicto, crecimiento, evolución reciente y caídas por segmento; Estrategia por canal no calcula participaciones sobre un porcentaje | `core/executive._monthly` · `core/dashboard_engine` · `core/insights` · `core/diagnostics._caida_por_segmento` · `ui/comercial.render_comercial` | semántica de la métrica | 🔴 — ocho jefes al 90% no son un 720%: el veredicto decía "alcanzó 667". |
...
| ~~2.15~~ | ~~Comparación individual embebida en Descripción (elegir dimensión, hasta 6 elementos, 9 tipos de gráfico)~~ **ELIMINADA** a pedido del usuario: la pestaña «📊 Cuadro comparativo» responde la misma pregunta sin tope de elementos, con meta, posición y la referencia del grupo completo, y «⚔️ Comparar personas» cubre el A vs B. Mantener las dos era mantener dos caminos para lo mismo | se quitó el bloque de `ui/dashboard._primary_analysis_section` y, con él, `_individual_trend` (sin otro llamador) | — | 🟢 — sustituida, no perdida. |
| ~~2.16~~ | ~~Ficha de persona embebida tras comparar 1 sola persona~~ **ELIMINADA** junto con 2.15: era la implementación duplicada que la auditoría ya señalaba (punto 3 de «Puntos de regresión»). La ficha real vive en `ui/person_profile.render_person_profile` (pestaña «🔎 Análisis de seguimiento») | se quitó `ui/dashboard._person_profile` | — | 🟢 — queda una sola implementación, que era el objetivo. |
...
| 8.6 | Analítica (modo Analista): siete lentes — 🏆 Ranking (participación, acumulado, cuántos hacen el 80%), 📈 Evolución por grupo (mes/trimestre/año/semana/día; valor, acumulado o índice base 100), ⚖️ Periodo vs. periodo (quién sumó y quién restó, aporte al cambio), 🧮 Tabla cruzada (dimensión × dimensión o × periodo; valores, % fila, % columna, % total), 🎯 Ejecutado vs. meta (pares PPTO/EJEC detectados por nombre), 🔗 Relación entre dos métricas (r, lo esperado, quién rinde más o menos de lo esperado) y 📦 Distribución (cajas, dispersión, atípicos). Cada lente: base del análisis, gráfico, «Qué dice» con nombres y cifras, tabla y CSV | `core/explorador.py` · render `ui/explorer.render_explorer` | `core/cuadro_comparativo.opciones_de_comparacion`, `core/performance.columna_meta`, `core/diagnostics` | 🔴 — reemplaza al explorador de «X contra Y» que (1) dejaba agrupar por métricas y fechas, (2) rotulaba todo con el concepto genérico («Cantidad») y no con el nombre de la columna, (3) sumaba ARPU y participaciones y (4) su «Evolución» ignoraba la dimensión. `calculo_automatico` promedia por NOMBRE (RATIO_RE: ARPU, PART, %, precio…) aunque el motor lo llame cantidad. `ultimo_incompleto` usa días Y volumen (<50% de la mediana) porque los cortes mensuales vienen fechados el día 1 y la señal por días no los ve; por volumen solo en mes/trimestre/año. Es POR MÉTRICA a propósito: en el archivo real, septiembre está completo en ALTAS y a medias en Altas Eje e Inducidas. `_frase_extremos` impide decir «el que más creció (−77%)». El aporte al cambio se divide entre el cambio CON signo para que sume 100%. Se quitó el filtro de texto propio sobre los valores de X: acotar se hace en el menú lateral o eligiendo grupos en Evolución. `tests/explorador_test.py`. |
...
| 11.9 | Los informes HTML (el de la selección, el de una hoja y el de TODO el Excel) traen además las secciones con las que se decide: «Cómo va cada uno» (cumplimiento vs. meta o promedio del grupo, con su gráfico, evolución, lectura y tabla), «Qué cambió entre los dos últimos periodos» (quién sumó y quién restó, con aporte al cambio), «Estrategia por canal» (semáforo con participación, movimiento y meta + jugadas con su impacto) y «Planes de mejora» (frentes en orden de atención, con pasos y con qué se cierra) | `ui/report_secciones.py` · llamadas desde `ui/report_html.build_html_report` y `build_workbook_html_report` | `core.cuadro_comparativo`, `core.explorador`, `core.comercial`, `core.planes` | 🟡 — no hay cálculo nuevo: cada sección usa el MISMO motor que su pestaña, y se descarta sola si el archivo no da para ella (sin fechas no hay cambio entre periodos; sin canal no hay estrategia). La métrica es la principal del informe: sin pasarla, el cuadro caía en «cantidad de registros» y comparaba cuántas filas tiene cada región. `tests/informe_html_test.py`. |
| 11.10 | El motor de gráficos (Plotly, ~3.5 MB) se incrusta UNA sola vez por documento | `ui/report_html._build_charts(incluir_motor=)` | — | 🔴 — en el informe de todo el Excel cada hoja metía su propia copia: 13 hojas = 58 MB, imposible de enviar por correo. Con una sola copia el mismo informe pesa 5.9 MB. El motor viaja en el PRIMER gráfico que se dibuje, sea de la hoja que sea: si la primera hoja no tiene gráficos, lo lleva el primero de la siguiente. |
